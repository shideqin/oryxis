//! `oryxis-cloud-aliyun-plugin`, the Alibaba Cloud provider as a standalone
//! subprocess.
//!
...
//! `aliyun`. It stays a separate subprocess only for architectural
...
async fn main() {
    tracing_subscriber::fmt()
        ...
        .with_env_filter(
            tracing_subscriber::EnvFilter::from_default_env()
                .add_directive("oryxis_cloud_aliyun_plugin=info".parse().unwrap()),
        )
        .init();

    tracing::info!(
        version = env!("CARGO_PKG_VERSION"),
        "oryxis-cloud-aliyun-plugin started"
    );
    ...
    tracing::info!("stdin closed, oryxis-cloud-aliyun-plugin exiting");
}
