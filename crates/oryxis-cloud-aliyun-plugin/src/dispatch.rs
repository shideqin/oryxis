//! Map JSON-RPC method calls onto `oryxis-cloud-aliyun`.
//!
//! Alibaba Cloud implements the discovery surface (credentials, discover,
//! transport-metadata query) plus `provider.cluster_kubeconfig` for ACK.
...
//! the app, and ACK clusters are served by the Kubernetes provider
//! once their kubeconfig is fetched.

use oryxis_cloud::CloudProvider;
use oryxis_cloud_aliyun::AliyunProvider;
...
mod tests {
    ...
    #[tokio::test]
    async fn initialize_returns_aliyun_metadata() {
        let resp = handle(
            method::INITIALIZE,
            Value::from(1),
        ...
        assert_eq!(init.provider_id, "aliyun");
        assert!(
            init.capabilities
