//! `oryxis-cloud-tencent-plugin`, the Tencent Cloud provider as a standalone
//! subprocess.
//!
//! Speaks line-delimited JSON-RPC 2.0 over stdio (the
//! `oryxis-plugin-protocol` contract), the same framing the AWS / K8s
//! plugins and `oryxis-mcp` use. The main app spawns this binary, runs the
//! `initialize` handshake, and drives discovery / credentials through it.
//!
//! Like the K8s plugin this carries no SDK: every operation shells out to
//! `tccli`. It stays a separate subprocess only for architectural
...
use oryxis_plugin_protocol::{JsonRpcRequest, JsonRpcResponse, error_codes, method};
...
async fn main() {
    tracing_subscriber::fmt()
        ...
        .with_env_filter(
            tracing_subscriber::EnvFilter::from_default_env()
                .add_directive("oryxis_cloud_tencent_plugin=info".parse().unwrap()),
        )
        .init();

    tracing::info!(
        version = env!("CARGO_PKG_VERSION"),
        "oryxis-cloud-tencent-plugin started"
    );
    ...
    tracing::info!("stdin closed, oryxis-cloud-tencent-plugin exiting");
}
